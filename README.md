...
| 김대호 | 서로 다른 전공을 가진 팀원들이 모여 진행한 이번 프로젝트는 협업의 가치를 깊이 깨닫는 계기가 되었습니다. 각자의 전문 지식을 결합하는 집단지성의 힘이 얼마나 강력한지 체감할 수 있었으며, 철저한 사전 계획 덕분에 전체 일정을 원활하게 소화할 수 있었습니다. 특히 팀원에게 내용을 설명하고 공유하는 과정은 스스로의 지식을 명확히 정리하고 다지는 데 큰 도움이 되었습니다. |
| 김진화 | (회고)각자의 페이지를 따로 만드는 프로세스 덕에 브랜치 관리가 중요했고, 이를 적절히 통합하는, 일종의 작은 시스템 통합 작업이 중요했는데, 여기서 시행착오를 겪으면서 알게 되는 것이 많았습니다. 그리고 프론트엔드를 다뤄보는 경험이 아예 없다시피 했는데, 프론트엔드 프레임워크에 대해서 겪어보면서 알게되는 경험이 인상적이었습니다.  (역할) 이탈 데이터 시각화 (별도 브랜치에서 작업 중, 메인과 충돌 문제로 인한 미병합), 기능 정상 작동 검증(일부), 데이터 탐색적 분석(초기) |
