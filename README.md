...
| 이름 | 담당 기능 |
|---|---|
| 황호순 | PM / 데이터 전처리 / 머신러닝 모델 학습<br/>모델 히스토리 페이지 / 문서 작성 |
| 김대호 | 프로토타입·배포(Docker/Railway), 기부자 관리(배치 예측·후속조치)<br/>AWS Bedrock AI 메시지 초안, ML 백엔드 |
| 김진화 | 이탈 데이터 시각화<br/>기능 정상 작동 검증(일부), 데이터 탐색적 분석(초기) |
| 채정석 | 예측 결과 기반 통계·솔루션 페이지 개발<br/>README 작성·문서화 |
| 홍지윤 | 사용자 인증(이메일·Google·Kakao)<br/>Firestore 기반 마이페이지·기부자 명단·활동 기록 |
